...
// These helper functions let us use async/await with SQLite instead of nested callbacks.
...
async function seedDatabase() {
  ...
  await ensureColumn("redemptions", "created_at", "TEXT");
  await run(
...
app.post("/points", auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    await run("UPDATE users SET points = ? WHERE id = ?", [nextPoints, target.id]);
